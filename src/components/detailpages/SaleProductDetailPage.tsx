...
import { getSaleProductById } from "@/app/service/salesproduct";
import Navbar from "@/components/common/Navbar";

async function SaleProductData({ id }: { id: string }) {
  try {
    const product = await getSaleProductById(id);
...
function SaleProductDetailLoading() {
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
...
export default function SaleProductDetailPage({ id }: { id: string }) {
  return (
    <>
      <Navbar />
      <div className="container mx-auto px-4  py-6 lg:py-16">
        <div className="max-w-6xl mx-auto">
          ...
          <Suspense fallback={<SaleProductDetailLoading />}>
            <SaleProductData id={id} />
          </Suspense>
        </div>
      </div>
    </>
